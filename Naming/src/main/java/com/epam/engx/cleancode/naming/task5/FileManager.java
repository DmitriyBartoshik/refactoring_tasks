...
public final class FileManager {
    ...
    private static final String[] IMAGE_FORMAT_TYPES = {"jpg", "png"};
    private static final String[] DOCUMENT_FORMAT_TYPES = {"pdf", "doc"};
    ...
    public List<String> getAllImagesList() {
        return addFiles(basePath, IMAGE_FORMAT_TYPES);
    }

    public List<String> getAllDocumentFilesList() {
        return addFiles(basePath, DOCUMENT_FORMAT_TYPES);
    }
    ...
    private boolean isInvalidImage(String fileName) {
        FileExtensionPredicate imageExtensionsPredicate = new FileExtensionPredicate(IMAGE_FORMAT_TYPES);
        return !imageExtensionsPredicate.test(fileName);
    }

    private boolean isInvalidDocument(String fileName) {
        FileExtensionPredicate documentExtensionsPredicate = new FileExtensionPredicate(DOCUMENT_FORMAT_TYPES);
        return !documentExtensionsPredicate.test(fileName);
    }

    private List<String> addFiles(String directoryPath, String[] allowedExtensions) {
        final FileExtensionPredicate predicate = new FileExtensionPredicate(allowedExtensions);
        return Arrays.asList(createDirectory(directoryPath).list(getFilenameFilterByPredicate(predicate)));
    }
